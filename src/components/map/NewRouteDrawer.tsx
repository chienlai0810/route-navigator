...
export function NewRouteDrawer({
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => { if (!v) handleCancel(); else onOpenChange(v); }}>
      <SheetContent side="right" className="w-[380px] sm:max-w-[380px] flex flex-col">
        <SheetHeader>
          <SheetTitle>Tạo tuyến mới</SheetTitle>
          <SheetDescription>
            Nhập thông tin cho tuyến vừa vẽ trên bản đồ.
          </SheetDescription>
        </SheetHeader>

        <div className="flex-1 space-y-5 py-4 overflow-y-auto custom-scrollbar">
          {/* Route Name */}
